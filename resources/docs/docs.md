...
The `home-routes` handler in the `upload-validate-java.routes.home` namespace
...
The `home-page` function will in turn call the `upload-validate-java.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `upload-validate-java.handler` namespace:
...
Request middleware functions are located under the `upload_validate_java.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `upload-validate-java.dev-middleware` namespace found in
